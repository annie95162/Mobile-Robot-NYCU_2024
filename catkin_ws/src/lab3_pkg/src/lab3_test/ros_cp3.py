...
def left_and_go():
    ...
    # Perform a left turn
    #publish_queue = [{'left': -rotate_speed, 'right': rotate_speed}]
    time.sleep(4.0)  # Perform left turn for a while to change direction
    
    # Go straight after turning left
    publish_queue = [{'left': speed, 'right': speed}]
    
    # Continue moving straight until left sensor detects an obstacle
    while state == "left_and_go":
        if sensor[1] == 0:  # Left sensor detects an obstacle again
            back()
            right_and_go()
            return
        time.sleep(0.01)
...
def right_and_go():
    ...
    publish_queue = [{'left': rotate_speed, 'right': -rotate_speed}]
    time.sleep(4.0)  # Perform right turn for a while to change direction
    ...
    while state == "right_and_go":
        if sensor[0] == 0:
          finish()
        elif sensor[1] == 0:  # Left sensor detects an obstacle for the third time
            back(0.10)
            right_and_go_until_front()
            return
        time.sleep(0.01)
