...
def publish_data_handeling_loop():
    ...
    # pub2 = rospy.Publisher('servo', Int16MultiArray, queue_size = 10)  # 註解掉servo的Publisher
    ...
    while not rospy.is_shutdown():
        if publish_queue:
            ...
            pub.publish(msg)
            rate.sleep()
        # elif publish_queue2:  # 註解掉 servo 的 publish queue
        #     msg = Int16MultiArray()
...
def init():
    ...
    # publish_queue2 = [{'servo': 0}]  # 註解掉 servo的操作
    is_locked = False
    not_catch_time = 0
...
def finish():
    ...
    publish_queue = [{'left': 0, 'right': 0}]
    time.sleep(1)
    # publish_queue2 = [{'servo': 0}]  # 註解掉 servo 的操作
    time.sleep(2)
    publish_queue = [{'left': - speed, 'right': - speed}]
