...
find_ir = False
find_r_ir = False
not_find_ir_threshold = 2000 # <-set it yourself
...
def left_and_go():
    ...
    global state, publish_queue, max_light, subscribe_data, subscribe_data_queue, find_ir, received_ir_data, find_r_ir, encoder_a_plus_b, light_data
    state = "left_and_go"
    flag = False
    # Perform a right turn
    # publish_queue = [{'left': -rotate_speed, 'right': rotate_speed}]
    ...
    while flag == False:
        publish_queue = [{'left': -(rotate_speed), 'right': rotate_speed}]
        if (sensor[1] == 0) and (received_ir_data > 0.1):
            flag = True
            finish()
            break
        elif (sensor[1] == 0) and (received_ir_data <= 0.1):
            back(0.2)
            ...
            encoder_a_plus_b = 0
            publish_queue = [{'left': -(rotate_speed), 'right': rotate_speed}]
            while (ARotate/2) > encoder_a_plus_b:    
                time.sleep(0.1)
            publish_queue = [{'left': speed, 'right': speed}]
            time.sleep(3.0)
            find_r_ir = False
...
def encoder_data_output_loop():
    global encoder_a_plus_b, motor_not_move, not_move_threshold
...
def ir_sensor_output():
    global received_ir_data, find_ir, find_r_ir
    ir_count0 = 0.0
    ir_count1 = 0.0
    ...
    while not rospy.is_shutdown():
        ...
        if (ir_count0 + ir_count1) >= read_ir_times:
            ...
            if (received_ir_data >= beacon_lower_bound[beacon_select]) and (received_ir_data <= beacon_upper_bound[beacon_select]):
                not_find_ir_counter = 0
                find_ir = True
            elif (received_ir_data >= beacon_lower_bound[beacon_select_r]) and (received_ir_data <= beacon_upper_bound[beacon_select_r]):
                not_find_ir_counter = 0
                find_r_ir = True
            else:
